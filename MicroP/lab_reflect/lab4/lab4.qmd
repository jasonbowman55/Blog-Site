...
The **minimum** duration corresponds to the maximum value of the 16-bit ARR register ($ARR_{\text{max}} = 2^{16} - 1$). This can be calculated using Equation [10] below.

[10] ${ms_{\text{desired}}} = \frac{65535}{100}$ = 655.35ms

The **maximum** duration corresponds to the smallest value of the 16-bit ARR register ($ARR_{\text{min}} = 1$). This can be calculated using Equation [11] below.
